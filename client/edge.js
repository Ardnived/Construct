...
edge.instance = function(q1, r1, q2, r2) {
	this.base = abstract_edge.instance;
	this.base(q1, r1, q2, r2);
	...
	this.height = Math.abs(board.hex.get_y(q1, r1) - board.hex.get_y(q2, r2)) * 1.2 / 5;
	this.x = board.edge.get_x(q1, r1, q2, r2);
	this.y = board.edge.get_y(q1, r1, q2, r2);
	this.rotation = board.edge.angle(q1, r1, q2, r2);
	this.hover = false;
	
	this._entity = Crafty.e("2D, Canvas, "+canvas.image.edge.neutral.normal)
		.attr({x: this.x - board.edge.offset.x, y: this.y - board.edge.offset.y, w: this.width, h: this.height, z:-1})
		.origin("center")
		.attr({rotation: this.rotation});

	this.image(canvas.image.edge.neutral);
	
	this.show();
};

edge.instance.prototype = new abstract_edge.instance;

edge.instance.prototype.owner = function(new_owner) {
	...
	if (change) {
		if (ui.players.is(this._owner, ui.players.left)) {
			this.image(canvas.image.edge.left);
		} else if (ui.players.is(this._owner, ui.players.right)) {
			this.image(canvas.image.edge.right);
		} else {
			this.image(canvas.image.edge.neutral);
		}
